 ...
 */
public abstract class ExtendableListViewFork extends ExtendableListView {

    private static final String TAG = "ExtendableListViewFork";
...
//    }
    ...
    public ExtendableListViewFork(final Context context, final AttributeSet attrs, final int defStyle) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                // FORKED Edge Glows
                if(mEdgeGlowTop.onRelease() | mEdgeGlowBottom.onRelease()) {
                    invalidate();
                }
                break;
...
//            case TOUCH_MODE_OVERSCROLL:
        ...
        if (mTouchMode == TOUCH_MODE_SCROLLING) {
            ...
            if (y != mLastY) {
                ...
                if (atEdge) {
                    ...
                    if (DBG) Log.d(TAG, "scrollIfNeeded - edge effect & overscroll");
                }
            ...
            if (DBG) Log.d(TAG, "scrollIfNeeded - support overscroll");
        }
    ...
    private boolean moveTheChildren(int deltaY, int incrementalDeltaY) {
        ...
        if (cannotScrollUp) {
            if (DBG) Log.d(TAG, "moveTheChildren cannotScrollUp " + cannotScrollUp + " : " +
                    ...
                    " maxBottom " + maxBottom +
                    " incrementalDeltaY " + incrementalDeltaY);
            mEdgeGlowBottom.onPull(0.5f, 0.5f);
            invalidate();
            return incrementalDeltaY != 0;
        }
    ...
    class AdapterDataSetObserver extends DataSetObserver {
        ...
        public void onChanged() {
            ...
            if (ExtendableListViewFork.this.getAdapter().hasStableIds() && mInstanceState != null
                    && mOldItemCount == 0 && mItemCount > 0) {
                ExtendableListViewFork.this.onRestoreInstanceState(mInstanceState);
                mInstanceState = null;
            }
            else {
                rememberSyncState();
        ...
        public void onInvalidated() {
            ...
            if (ExtendableListViewFork.this.getAdapter().hasStableIds()) {
                // Remember the current state for the case where our hosting activity is being
                // stopped and later restarted
                mInstanceState = ExtendableListViewFork.this.onSaveInstanceState();
            }
    ...
    private boolean performLongPress(final View child,
        ...
        if (onItemLongClickListener != null) {
            handled = onItemLongClickListener.onItemLongClick(ExtendableListViewFork.this, child,
...
//        }
    ...
    public void draw(Canvas canvas) {
        ...
        if (!mEdgeGlowTop.isFinished()) {
            ...
            mEdgeGlowTop.setSize(height, width);
            if (mEdgeGlowTop.draw(canvas)) invalidate();
        ...
        if (!mEdgeGlowBottom.isFinished()) {
            ...
            final int height = getHeight() - getPaddingTop() - getPaddingBottom();
            canvas.rotate(180);
            canvas.translate(-100, -width);
            mEdgeGlowBottom.setSize(height, width);
            if (mEdgeGlowBottom.draw(canvas)) invalidate();
            canvas.restoreToCount(restoreCount);
        }
    }
